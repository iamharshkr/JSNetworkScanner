...
if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(
            description="Fetch JavaScript files from a website and find HTTP requests.")
        parser.add_argument(
            "url", help="URL of the website to fetch JavaScript files from")
        parser.add_argument(
            "-f", "--file", help="Path to a text file containing URLs (one per line)")
        parser.add_argument(
            "-o", "--output", help="Path to the output text file")
        parser.add_argument(
            "-e", "--exclude", help="Comma-separated list of additional library names to exclude")
        ...
        if args.file:
            ...
            urls = [args.url]
        excluded_libraries = ["jquery", "bootstrap",
                              "cloudflare", "datatables"]  # Default exclusions
        if args.exclude:
            # Extend with additional exclusions
            excluded_libraries.extend(args.exclude.lower().split(','))
        for url in urls:
            js_files = ''
            ...
            if js_files:
                ...
                for js_file in js_files:
                    ...
                    print(full_js_url)
                    response = fetch_url(full_js_url)
                    if response.status_code == 200:
                    ...
                    for js_file in js_files:
                        ...
                        if response.status_code == 200:
                            ...
                            if request_methods:
                                ...
                                for method, req_url in request_methods:
                                    output_file.write(
                                        f"{method.upper()} request: {req_url}\n")
                        else:
                            output_file.write(
                                f"Failed to fetch {full_js_url}. Status code: {response.status_code}\n")
